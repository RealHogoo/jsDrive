...
- 기존 파일 SHA-256 해시 백필
- 영상 720p/1080p 변환과 HLS 조각 생성
- 미디어 서비스용 내부 Range/HLS 스트리밍
...
- 미디어 서비스 내부 API
  - `POST /internal/media/list.json`
  - `POST /internal/media/file-detail.json`
  - `POST /internal/media/file-stream.json`
  - `POST /internal/media/hls-stream.json`
...
웹하드가 실제 파일 저장소를 소유하므로 트랜스코딩과 HLS 조각 생성도 웹하드 서비스에서 수행합니다.

동작 흐름:

1. 영상 파일을 `wh_transcode_job`에 `PENDING`으로 등록합니다.
2. 허용 시간대에 웹하드 프로세스가 ffmpeg를 실행합니다.
3. 1080p/720p H.264 + AAC MP4를 만들고 `wh_transcode_variant`에 기록합니다.
4. 각 variant로 HLS playlist와 `.ts` segment를 만들고 `wh_hls_rendition`에 기록합니다.
5. 미디어 서비스는 `/internal/media/hls-stream.json`을 호출해 HLS 파일을 프록시합니다.

기본 운영값은 새벽 시간대에 보수적으로 동작하도록 되어 있습니다.

| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `WEBHARD_TRANSCODE_ENABLED` | `true` | 트랜스코딩 사용 여부 |
...
| `WEBHARD_TRANSCODE_AUDIO_BITRATE` | `160k` | AAC 비트레이트 |
| `WEBHARD_HLS_SEGMENT_SECONDS` | `4` | HLS segment 길이 |

원본 MP4 스트리밍은 `Range` 요청을 지원하며 정상 seek 요청은 `206 Partial Content`로 응답합니다. HLS는 `master.m3u8`, variant playlist, `.ts` segment를 작은 조각으로 제공해 초기 로딩과 네트워크 변동에 대응합니다.
